...
>
	<div class="grid w-full h-full">
		<div class={`col-start-1 row-start-1 ${gameState.isGameOver ? 'z-1' : 'z-10'}`}>
			<div class={`${gameState.isGameOver ? 'animate-blurout opacity-0' : ''}`}>
				<Grid bind:gameState />
			</div>
		</div>
		<div class={`col-start-1 row-start-1 ${gameState.isGameOver ? 'z-10' : 'z-1'}`}>
